...
**本项目是抖音云 Serverless 项目，主要部署方式不需要 Docker！**

- ✅ **技术栈**: TypeScript + Node.js + 抖音云函数
- ❌ **不是**: Java Spring Boot 或其他容器化应用
- ⚠️ **Dockerfile**: 仅作为 CI/CD 流水线占位符，实际不使用

---

### 📌 关于 Dockerfile 的说明

项目中存在 `Dockerfile` 文件的原因是：

1. **抖音云持续交付流水线要求** - 流水线默认配置需要 Dockerfile
2. **占位用途** - 该文件仅用于满足流水线配置，不会被实际使用
3. **实际部署** - 仍然通过上传云函数代码的方式部署

**如果您不使用持续交付流水线，可以忽略此文件。**
